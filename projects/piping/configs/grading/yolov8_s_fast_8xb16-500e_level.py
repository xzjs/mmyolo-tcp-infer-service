...
custom_imports = dict(imports = ['piping_transform',
                                 'piping_utils',
                                 'piping_preprocessors',
                                 'yolov8head_with_level',
                                 'grading_module',
                                 'yolo_piping',
                                 'piping_metrics',
                                 'piping_hooks',
                                 'piping_yolov5_optim_constructor',
                                 'piping_coco'], allow_failed_imports=False)

load_from = 'https://download.openmmlab.com/mmyolo/v0/yolov8/yolov8_s_mask-refine_syncbn_fast_8xb16-500e_coco/yolov8_n_mask-refine_syncbn_fast_8xb16-500e_coco_20230216_101206-b975b1cd.pth'
...
train_ann_file = 'annotations/annotations_v4/train_normal_coco_format_with_level.json'
train_data_prefix = 'images/'  # Prefix of train image path
# Path of val annotation file
val_ann_file = 'annotations/annotations_v4/test_coco_format_with_level.json'
...
dataset_type = 'PipingCocoDataset'
...
model = dict(
    type='YOLOGradeDetector',
    data_preprocessor=dict(
        type='PipingYOLOv5DetDataPreprocessor',
        mean=[0., 0., 0.],
        std=[255., 255., 255.],
        bgr_to_rgb=True),
    backbone=dict(
...
pre_transform = [
    ...
    dict(type='LoadAnnotations_with_level', with_bbox=True, with_level=True)
]
...
train_dataloader = dict(
    ...
    collate_fn=dict(type='piping_yolov5_collate'),
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
        metainfo=metainfo,
...
test_pipeline = [
    ...
    dict(
        ...
        meta_keys=('img_id', 'img_path', 'ori_shape', 'img_shape',
                   'scale_factor', 'pad_param', 'gt_level'))
]
...
optim_wrapper = dict(
    ...
    constructor='PipingYOLOv5OptimizerConstructor')
...
default_hooks = dict(
    ...
    logger=dict(type='LoggerHook', interval=log_intervals))
